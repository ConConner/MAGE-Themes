            ...
            protected override void WndProc(ref Message m)
            {
                if (m.Msg == 0xF /*WM_PAINT*/ && ((FlatNumericUpDown)updown.Parent).BorderStyle == BorderStyle.FixedSingle)
                {
                    ...
                    using (var g = Graphics.FromHdc(s.hdc))
                    {
                        ...
                        using (var backBrush = new SolidBrush(((FlatNumericUpDown)updown.Parent).BackColor))
                        {
                        ...
                        Brush arrowBrush = new SolidBrush(updown.Enabled ? ThemeSwitcher.ProjectTheme.TextColor : ThemeSwitcher.ProjectTheme.TextColorDisabled);
                        g.FillPolygon(arrowBrush, GetUpArrow(r1));
                        g.FillPolygon(arrowBrush, GetDownArrow(r2));
                    }
                    m.Result = (IntPtr)1;
                    base.WndProc(ref m);
